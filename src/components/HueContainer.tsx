...
interface Props {
  ...
  toggleHueLiked: (hueId: number, userId?: string) => void;
}

const HueContainer = ({ posts, handleDeleteHue, toggleHueLiked }: Props) => {
      ...
      {posts?.map((post) => {
        return (
          <Post
            ...
            handleLikeUnlike={(hueId) => toggleHueLiked(hueId, account?.id)}
